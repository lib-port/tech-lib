...
1. Set variables for the account and group.
...
1. Check whether the user or group already exists.
...
1. Add the new user to the Accounting group.
...
1. Enter a secure password when prompted. Complete the requested name and contact fields, or press Enter to leave optional fields blank. Review the summary, and enter `Y` if the information is correct.
2. Display newly created account with the assignment's required command. Anchoring the search to `velma:` avoids matching a different account that contains the same text.
...
1. Check whether the required names already exist.

```bash
ls -ld Accounting Payroll 2>/dev/null
```

1. If neither name exists, create both folders.

```bash
mkdir Accounting Payroll
```

1. Display new folders.

```bash
ls -ld Accounting Payroll
```

1. Create a file named `Best Practices` inside the Accounting folder. Quotation marks are required because the filename contains a space.

```bash
touch "Accounting/Best Practices"
```

1. Display contents of the Accounting folder.
...
1. Review packages that have upgrades available.
...
1. Read the proposed changes before confirming.
2. Display recent package-management log with the required command.

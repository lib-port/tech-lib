...
class Product(db.Model):
    ...
    def deserialize(self, data: dict):
        ...
        try:
            ...
            if not isinstance(self.name, str) or not 1 <= len(self.name.strip()) <= 100:
                raise DataValidationError("Name must contain between 1 and 100 characters")
            if not isinstance(self.description, str) or len(self.description) > 250:
                raise DataValidationError("Description must be a string of at most 250 characters")
            if not self.price.is_finite() or not 0 <= self.price <= Decimal("99999999.99"):
                raise DataValidationError("Price must be between 0 and 99999999.99")
            ...
            raise DataValidationError("Price must be a valid number") from error
        return self
    ...
    def find_by_category(cls, category: Category = Category.UNKNOWN) -> list:
        ...
        :param category: a member of the Category enum
        :type available: enum
